...
def parse_nets(sch_text, components):
    ...
    # net_at_pos: {(x,y): net_name} — for any position reachable from a label
    net_at_pos = {}
        ...
        while queue:
            ...
            for nb in wire_graph.get(pos, []):
                if nb not in visited:
                    visited.add(nb)
                    queue.append(nb)
                    # Also check if neighbour has a label
                    if nb in labels:
...
def assign_nets_to_board(board, components, net_at_pos, config_net_assignments=None):
    ...
    Priority:
    1. config_net_assignments dict: {ref: {pad_name: net_name}} — deterministic, from schematic_config.json
    2. pad-position lookup in net_at_pos — approximate, schematic-coordinate based
    3. component-center lookup — last resort fallback
    """
    import pcbnew
    assigned = 0
    net_cache = {}  # net_name -> NETINFO_ITEM
    ...
    # Build ref -> schematic position map (in mm)
    ...
    for fp in board.GetFootprints():
        ...
        for pad in fp.Pads():
            ...
            # Priority 3: component center fallback
            if not net_name and ref in comp_pos:
                sx, sy = comp_pos[ref]
...
def sync(sch_path, pcb_path):
    ...
    # Load existing PCB to preserve library tables (critical for FootprintLoad)
    ...
    if config_path.exists():
        try:
            ...
            if config_net_assignments:
                print(f'[sync] Using net_assignments from schematic_config.json ({len(config_net_assignments)} components)')
        except Exception as e:
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print('Usage: DISPLAY=:99 python3 sch_to_pcb_sync.py <sch> <pcb>')
        sys.exit(1)
